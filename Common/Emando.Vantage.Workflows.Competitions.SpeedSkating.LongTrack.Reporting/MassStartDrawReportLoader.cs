...
{
    public class MassStartDrawReportLoader : IDistanceReportLoader
    {
        private readonly Func<ICompetitionContext> contextFactory;

        public MassStartDrawReportLoader(Func<ICompetitionContext> contextFactory)
        {
        ...
        internal static async Task<MassStartDrawReport> LoadAsync(ICompetitionContext context, Guid competitionId, Guid distanceId,
            OptionalReportColumns optionalColumns)
        {
            var report = new MassStartDrawReport();
